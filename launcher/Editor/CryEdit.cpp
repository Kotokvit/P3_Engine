...
public:
    ...
    CEditCommandLineInfo()
    {
        ...
        parser.setApplicationDescription(QObject::tr("P3Engine Editor"));
...
{
    const QString informativeMessage = QObject::tr("Please follow the instructions <a href=\"https://www.p3engine.org/docs/user-guide/project-config/add-remove-gems/\">here</a>, after which the Editor will be re-launched automatically.");
...
QString FormatRichTextCopyrightNotice()
{
    ...
    QString copyrightString = QObject::tr("Copyright %1 Contributors to the P3 Engine Project");
...
AZ::Outcome<void, AZStd::string> CCryEditApp::InitGameSystem(HWND hwndForInputSystem)
{
    ...
    // needs to be called after P3System has been loaded.
...
bool CCryEditApp::InitInstance()
{
    ...
    mainWindowWrapper->enableSaveRestoreGeometry("P3Engine", "P3Engine", "mainWindowGeometry");
...
{
    QString webLink = tr("https://p3engine.org/docs/learning-guide/");
    QDesktopServices::openUrl(QUrl(webLink));
}

void CCryEditApp::OnDocumentationGlossary()
{
    QString webLink = tr("https://p3engine.org/docs/user-guide/appendix/glossary/");
    QDesktopServices::openUrl(QUrl(webLink));
}

void CCryEditApp::OnDocumentationO3DE()
{
    QString webLink = tr("https://p3engine.org/docs/");
    QDesktopServices::openUrl(QUrl(webLink));
}

void CCryEditApp::OnDocumentationReleaseNotes()
{
    QString webLink = tr("https://p3engine.org/docs/release-notes/");
    QDesktopServices::openUrl(QUrl(webLink));
}

void CCryEditApp::OnDocumentationGameDevBlog()
{
    QString webLink = tr("https://p3engine.org/news-blogs/");
    QDesktopServices::openUrl(QUrl(webLink));
}

void CCryEditApp::OnDocumentationForums()
{
    QString webLink = tr("https://discord.com/invite/p3engine");
    QDesktopServices::openUrl(QUrl(webLink));
}
...
void CCryEditApp::OpenProjectManager(const AZStd::string& screen)
{
    ...
    if (!launchSuccess)
    {
        QMessageBox::critical(AzToolsFramework::GetActiveWindow(), QObject::tr("Failed to launch P3Engine Project Manager"), QObject::tr("Failed to find or start the O3dE Project Manager"));
    }
}
...
CCryEditApp::ECreateLevelResult CCryEditApp::CreateLevel(const QString& templateName, const QString& levelName, QString& fullyQualifiedLevelName /* ={} */, const QString& levelsRootAbsolutePath /* ={} */)
{
    ...
    P3SystemEventBus::Broadcast(&P3SystemEventBus::Events::OnCryEditorBeginCreate);
    ...
    GetIEditor()->Notify(eNotify_OnEndCreate);
    P3SystemEventBus::Broadcast(&P3SystemEventBus::Events::OnCryEditorEndCreate);
    return ECLR_OK;
}
...
void CCryEditApp::SetEditorWindowTitle(QString sTitleStr, QString sPreTitleStr, QString sPostTitleStr)
{
    if (MainWindow::instance() || m_pConsoleDialog)
    {
        ...
        {
            sTitleStr = QObject::tr("P3Engine Editor [%1]").arg(FormatVersion(m_pEditor->GetFileVersion()));
        }
...
extern "C" int AZ_DLL_EXPORT CryEditMain(int argc, char* argv[])
{
    ...
    // open a scope to contain the AZToolsApp instance;
    {
        ...
        AZ_Error("Editor", didCryEditStart, "P3Engine Editor did not initialize correctly, and will close."
