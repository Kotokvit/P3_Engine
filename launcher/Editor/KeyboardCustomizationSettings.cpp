...
{
    QSettings settings(QStringLiteral("P3Engine"), QStringLiteral("P3Engine"));
...
{
    QString fileName = QFileDialog::getSaveFileName(parent, QObject::tr("Export Keyboard Shortcuts"), QStringLiteral("p3engine.keys"), QObject::tr("Keyboard Settings (*.keys)"));
    if (fileName.isEmpty())
    {
        return;
    }
    ...
    store.insert("Content-Type", "application/x-p3engine-sdk-keyboard-settings+json");
    QJsonObject groups;
...
void KeyboardCustomizationSettings::ImportFromFile(QWidget* parent)
{
    ...
    if (store.value("Content-Type") != "application/x-p3engine-sdk-keyboard-settings+json" || store.value("version") != "1.0")
    {
