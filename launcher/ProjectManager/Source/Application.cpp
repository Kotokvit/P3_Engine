...
namespace P3::ProjectManager
{
    ...
    {
#if defined(EXTERNAL_CRASH_REPORTING)
        CrashHandler::ToolsCrashHandler::InitCrashHandler("p3engine", {});
#endif

        constexpr const char* applicationName { "P3Engine" };

        QApplication::setOrganizationName(applicationName);
        QApplication::setOrganizationDomain("p3engine.org");
        ...
        // set the log alias to .p3engine/Logs instead of the default user/logs
        ...
        // Save event logs to .p3engine/Logs/eventlogger/EventLogO3DE.azsl
        ...
        if (!m_pythonBindings->PythonStarted())
        {
            if (interactive)
            {
                ...
                QObject::tr("This tool requires an P3Engine engine with a Python runtime, "
                            "but was unable to automatically install P3Engine's built-in Python."
                            "You can troubleshoot this issue by trying to manually install P3Engine's built-in "
    ...
    bool Application::InitLog(const char* logName)
    {
        if (!m_entity)
        {
            ...
            // so the log will be named P3Engine.log
    ...
    bool Application::Run()
    {
        ...
        wrapper->enableSaveRestoreGeometry("P3Engine", "ProjectManager", "mainWindowGeometry");
