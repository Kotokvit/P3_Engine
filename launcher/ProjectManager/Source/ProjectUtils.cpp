...
namespace P3::ProjectManager
{
    namespace ProjectUtils
    {
        ...
        static bool WarnDirectoryOverwrite(const QString& path, QWidget* parent)
        {
            if (!QDir(path).isEmpty())
            {
                QMessageBox::StandardButton warningResult = QMessageBox::warning(
                    parent, QObject::tr("Перезаписать директорию"),
                    QObject::tr("Директория не пуста! Вы уверены, что хотите перезаписать?"), QMessageBox::No | QMessageBox::Yes);
                    ...
                    if (force || pInfo.IsSuccess())
                    {
                        if (pInfo.IsSuccess())
                        {
                            ...
                            if (!pInfo.GetValue().m_restricted.isEmpty())
                            {
                                ...
                                if (restrictedDirectory.cd("P3Engine/Restricted/Projects") &&
            ...
            const QString& title)
        {
            ...
            QProgressDialog dialog(title, QObject::tr("Отмена"), /*minimum=*/0, /*maximum=*/0);
