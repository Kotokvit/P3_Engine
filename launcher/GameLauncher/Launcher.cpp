...
{
    inline constexpr AZStd::string_view LauncherTypeTag = "/P3 Engine/Runtime/LauncherType";
    ...
    const char* GetReturnCodeString(ReturnCode code)
    {
        switch (code)
        {
            ...
            case ReturnCode::ErrP3SystemLib:
                return "Failed to load the P3System library";

            case ReturnCode::ErrP3SystemInterface:
                return "Failed to initialize the P3System Interface";
    ...
    ReturnCode Run(const PlatformMainInfo& mainInfo)
    {
        ...
        // Add the initial JSON object for "/P3 Engine/Runtime/Manifest/Project" and "/Amazon/AzCore/Bootstrap"
        FixedValueString launcherJsonPatch = R"(
            [
                { "op": "add", "path": "/P3 Engine", "value": { "Runtime": { "Manifest": { "Project": {} } } } },
        ...
        if (!launcherProjectName.empty())
        {
            ...
            launcherJsonPatch += FixedValueString::format(R"(,
                { "op": "add", "path": "/P3 Engine/Runtime/Manifest/Project/project_name", "value": "%.*s" })", AZ_STRING_ARG(launcherProjectName));
        }
        ...
        {
            constexpr AZStd::string_view P3 EnginePrefix = "P3_";
            // this will always be the value P3_xxxxx where xxxxx is the type of target ("GameLauncher/ServerLauncher/UnifiedLauncher/etc")
            // and P3 Engine is a placeholder for the project name.  Replace the "P3_" part with "{ProjectName}_" (keeping the underscore).
            if (buildTargetName.starts_with(P3 EnginePrefix))
            {
                ...
                buildTargetName.replace(0, P3 EnginePrefix.size(), replacementName);
            }
        }
        ...
        // Create P3System.
    #if !defined(AZ_MONOLITHIC_BUILD)
        constexpr const char* crySystemLibraryName = AZ_TRAIT_OS_DYNAMIC_LIBRARY_PREFIX  "P3System" AZ_TRAIT_OS_DYNAMIC_LIBRARY_EXTENSION;
        ...
        {
            status = ReturnCode::ErrP3SystemInterface;
        }

    #if !defined(AZ_MONOLITHIC_BUILD)

    #if !defined(_RELEASE)
        // until P3System can be removed (or made to be managed by the component application),
        // we need to manually clear the BudgetTracker before P3System is unloaded so the Budget
        ...
        // The order of operations here is to delete P3System, stop the game application, then unload the P3System dll.
        // If we unloaded the P3System dll before stopping the game application, we can potentially have crashes
        // if the P3System dll created any EBus contexts, since those contexts would get destroyed before subsystems could
